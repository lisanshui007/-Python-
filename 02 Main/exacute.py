...
class MainWindow(QWidget):
    ...
    def recordNames(self):
        if self.set_name.issubset(self.record_name1):  # 如果self.set_names是self.record_names 的子集返回ture
            ...
            if names_num > 0:
                ...
                self.lineTextInfo2 = []
                # 打开数据库连接
                db2 = pymysql.connect(host='localhost', user="root", password="mysql123456", database='facerecognition')
                # 使用cursor()方法获取操作游标
                cursor2 = db2.cursor()
                # 获取系统时间，保存到秒
                import datetime
    ...
    def checkNums(self):
        ...
        try:
            # 打开数据库连接
            db = pymysql.connect(host='localhost', user="root", password="mysql123456", database='facerecognition')
            # 使用cursor()方法获取操作游标
            cursor = db.cursor()
